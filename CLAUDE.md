...
└── r6_navigator/
    ...
    ├── main.py                  # Entry point: init DB, launch navigator UI
    │
    ├── db/
    │   ├── database.py          # Engine, session factory, init_db() + migrations
    │   └── models.py            # All SQLAlchemy mapped classes (navigator + missions)
    │
    ├── shared/                  # Shared utilities (no UI, no domain logic)
    │   ├── backup.py            # Save/restore DB (file copy + integrity check)
    │   └── ollama_client.py     # Low-level HTTP wrapper around the Ollama API
    │
    ├── navigator/               # R6 referential management sub-application
    │   ├── services/
    │   │   ├── crud.py              # All DB read/write for navigator entities
    │   │   ├── export_docx.py       # DOCX generation via python-docx
    │   │   ├── ai_generate.py       # Ollama generation (5 sections, retry logic)
    │   │   ├── ai_judge.py          # 3-judge LLM evaluation (parallel threads)
    │   │   └── prompt/              # Prompt files (.txt)
    ...
    │   │       ├── translate_*.txt  # 4 translation prompts
    │   │       ├── judge_*.txt      # 9 judge prompts (3 criteria × 3 sections)
    │   │       ├── halliday_rules.json  # Level-specific Halliday transitivity rules
    ...
    │       ├── detailpanel.py       # Right QTabWidget + isomorphism bar
    │       ├── tabfiche.py          # Tab: fiche fields + AI workers via QThread
    │       ├── tabquestions.py      # Tab: questions + observable items QTableWidgets
    │       ├── tabcoaching.py       # Tab: free-text coaching fields
    │       ├── dialogs.py           # Confirm dialogs, DOCX export config dialog
    │       ├── verification_window.py  # 3-judge LLM evaluation results window
    │       └── forms/               # Qt Designer .ui files + generated ui_*.py
    │           ├── tabfiche.ui      # Contains btn_generer + btn_generer_risque
    │           ├── tabquestions.ui  # Contains btn_generer + btn_generer_items
    │           ├── tabcoaching.ui
    │           └── ui_*.py          # Generated by pyside6-uic — do not edit manually
    │
    ├── missions/                # Mission interpretation sub-application
    │   ├── services/
    │   │   ├── crud.py              # All DB read/write for mission entities
    │   │   ├── export_docx.py       # DOCX export for mission reports
    │   │   ├── ai_analyze.py        # analyze_verbatim() + generate_mission_report()
    │   │   └── prompt/              # Mission-specific prompt files
    │   │       ├── system_01.txt    # System prompt (shared copy)
    │   │       ├── analyze_verbatim.txt
    │   │       └── generate_mission_report.txt
    │   └── ui/
    │       ├── app.py               # MissionApp (QMainWindow)
    │       ├── nav.py               # Left QTreeWidget: Mission → Interview → Verbatim
    │       ├── detail.py            # Right 4-tab orchestrator
    │       ├── tab_info.py          # Tab: mission metadata
    │       ├── tab_verbatim.py      # Tab: verbatim text + AI analysis worker
    │       ├── tab_interpretations.py  # Tab: validate/reject/correct interpretations
    │       ├── tab_rapport.py       # Tab: generate + export mission report
    │       └── forms/               # Manually-written ui_*.py (no .ui source)
    │           ├── ui_tab_mission_info.py
    │           ├── ui_tab_verbatim.py
    │           ├── ui_tab_interpretations.py
    │           └── ui_tab_rapport.py
    │
    ├── i18n/
    │   ├── __init__.py          # Lang singleton, t("key") helper
    │   ├── fr.json              # French UI strings (navigator + missions keys)
    │   └── en.json              # English UI strings (navigator + missions keys)
    │
    └── tests/
        ├── navigator/
        ...
        │   └── test_export.py
        └── missions/
            ├── conftest.py      # Mission-specific DB fixture
            └── test_mission_crud.py
```
...
- No DB access in UI files — all persistence goes through `navigator/services/crud.py` or `missions/services/crud.py`
...
- `EditGuard` in `navigator/ui/app.py` prevents navigating away from unsaved changes (confirm dialog)
...
`r6_navigator/navigator/services/prompt/system_01.txt` via `_load_system_prompt()`.

`ai_generate.py` reads `params.yml` via `Path(__file__).parent.parent.parent.parent / "params.yml"`.
...
```
navigator/ui/forms/*.ui     Qt Designer XML — edit with pyside6-designer, never by hand
    │
    ▼ pyside6-uic (compile step, run manually or via cli/compile_ui.py)
navigator/ui/forms/ui_*.py  Generated Python — widget declarations + setupUi() — do not edit
    │
    ▼ multiple inheritance mixin
navigator/ui/*.py           Logic classes — business behaviour, signal wiring, DB calls
```

Mission forms (`missions/ui/forms/ui_*.py`) are written manually (no `.ui` source).
...
```bash
pyside6-uic r6_navigator/navigator/ui/forms/tabfiche.ui \
    -o r6_navigator/navigator/ui/forms/ui_tabfiche.py
```

Or recompile all at once via the CLI helper:
...
3. **`_migrate_add_mission_tables()`** — no-op guard: `create_all()` handles table creation;
   this function is only a sentinel to detect already-migrated databases.

All migrations run transparently on application startup and are safe to run repeatedly.
